...
function App () {
  ...
  async function getBalance (asset) {
    const _balance = await asset.balance()
    setBalance(_balance)
  }
  ...
  async function generateDividendProof (zA, zB) {
    ...
    const residualNote = await note.create(acts[1].publicKey, 1, '0xf637cfb0c6be07eb0533d1600c7c3fe28df887a3')
  ...
  async function createStream (_web3, noteForStreamContract, startTime, endTime) {
    const streamContractInstance = new _web3.eth.Contract(streamContract.abi, streamContractAddress)
    console.log(streamContractInstance.methods)
    streamContractInstance.methods.createStream(payeeAddress, noteForStreamContract.noteHash, zkAssetAddress, startTime, endTime).call({}, (err, streamID) => {
      if (err) {
        console.log(err)
      } else {
