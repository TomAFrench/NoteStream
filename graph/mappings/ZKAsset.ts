...
function parseNoteAccessFromMetadata(metadata: Bytes): Map<string, Bytes> {
  ...
  if (metadataStr.length > METADATA_PREFIX_LEN) {
    let addressLenStr = stripLeadingZeros(
      metadataStr.slice(
        METADATA_PREFIX_LEN,
    ...
    for (let i = 0; i < addressCount; i += 1) {
      ...
      let address = metadataStr.slice(
        addressOffset,
        addressOffset + METADATA_ADDRESS_LEN
      );
      let viewingKeyOffset = addressEnd + i * METADATA_VIEWING_KEY_LEN;
      let viewingKey = metadataStr.slice(
        viewingKeyOffset,
        viewingKeyOffset + METADATA_VIEWING_KEY_LEN
      );
      accessMap.set("0x".concat(address), <Bytes>(
        Bytes.fromHexString(viewingKey)
      ));
    }
  }
  return accessMap;
...
export function createNote(event: CreateNote): void {
  ...
  createAccessFromMetadata(timestamp, note, metadata, <Bytes>(
    Bytes.fromHexString("")
  ));
}
