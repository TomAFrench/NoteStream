...
module.exports = (chai) => {
  ...
  const convert = (value) => {
    ...
    if (typeof value === 'string' || typeof value === 'number') {
      ...
      new chai.Assertion(value).assert(
        false,
        `expected ${value} to be an instance of string, number or BigNumber`,
      );
    }
  ...
  /**
   * Performs a boundary check instead of an equality check.
   * In real life circumstances,it can take up to 14 seconds for a block
   * to be broadcast on the Ethereum network, so we have to account for this.
   *
    ...
    if (tolerateByAddition) {
      ...
      return this.assert(
        actual.isGreaterThanOrEqualTo(expected) &&
          actual.isLessThanOrEqualTo(expectedCeiling),
      ...
      return this.assert(
        actual.isLessThanOrEqualTo(expected) &&
          actual.isGreaterThanOrEqualTo(expectedFloor),
